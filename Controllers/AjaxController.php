...
class AjaxController
{
    ...
    public static function validationNewPostAjax()
    {
        ...
        foreach ($postParams as $fieldName => $fieldValue) {
            if ($fieldName == "title") {
                ...
                if (!preg_match("/^[a-zA-Z0-9,.;:()-_' ]*$/",$fieldValue)) {
                    $errors[$fieldName] = "Title must contains only letters and whitespaces.";
                }
            ...
            if ($fieldName == "body") {
                ...
                if (!preg_match("/^[a-zA-Z0-9,.;:()-_' ]*$/",$fieldValue)) {
                    $errors[$fieldName] = "Body must contains only letters and whitespaces.";
                }
        ...
        if (empty($errors)) {
        $postParams["title"] = test_input($postParams["title"]);
        $postParams["body"]=test_input($postParams["body"]);

            try {
                Post::addNewPost($postParams);
                $result = array('status' => 'success','message' => $postParams['title']);
            } catch (\Exception $e) {
